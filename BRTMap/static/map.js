    ...
    function loadStationMarkers() {
    	...
    	$.getJSON("/static/estacoes_lotezero.json", function(json) {
    	    for(var i in json){
    	    	var lat = parseFloat(json[i].latitude);
    	    	var lng = parseFloat(json[i].longitude);
    	    	var marker = new google.maps.Marker({
    	...
    	$.getJSON("/static/estacoes_transolimpica.json", function(json) {
    	    for(var i in json){
    	    	var lat = parseFloat(json[i].latitude);
    	    	var lng = parseFloat(json[i].longitude);
    	    	var marker = new google.maps.Marker({
    ...
    function createChart() {
    	createSpeedPerWeek();
    	createSpeedPerDay();
    	createAvailablePerWeek();
    	createAvailablePerDay();
    }
    
    function createSpeedPerWeek(){
    	var request = $.ajax({
    		  url: "/averagespeedperweek",
    		  type: "POST",
    		  dataType: "json"
    		});
        	request.done(function(result) {
        		console.log(result);
        		$("#chartSpeedPerWeek").kendoChart({
                    title: {
                        text: "Velocidade média durante 7 dias"
                    },
                    legend: {
                        position: "top"
                    },
    ...
    function createSpeedPerDay(){
    	var request = $.ajax({
  		  url: "/averagespeedperday",
  		  type: "POST",
  		  dataType: "json"
  		});
    	request.done(function(result) {
    		console.log(result);
	    	$("#chartSpeedPerDay").kendoChart({
	            title: {
	                text: "Velocidade média durante 24 horas"
	            },
	            legend: {
	                position: "top"
	            },
    ...
    function createAvailablePerWeek(){
    	var request = $.ajax({
  		  url: "/availableperweek",
  		  type: "POST",
  		  dataType: "json"
  		});
    	request.done(function(result) {
    		console.log(result);
	    	$("#chartAvailablePerWeek").kendoChart({
	            title: {
	                text: "Ônibus disponível durante 7 dias"
	            },
	            legend: {
	                position: "top"
	            },
    ...
    function createAvailablePerDay(){
    	var request = $.ajax({
  		  url: "/availableperday",
  		  type: "POST",
  		  dataType: "json"
    	...
    	request.done(function(result) {
    		console.log(result);
	    	$("#chartAvailablePerDay").kendoChart({
	            title: {
	                text: "Ônibus disponível durante 24 horas"
	            },
	            legend: {
	                position: "top"
	            },
